...
class WorkerTest:
    def __init__(self, metaAgentID, localNetwork, global_step, budget_range, sample_length=None, device='cuda', greedy=False, save_image=False, seed=None, test_number=None):
    ...
    def run_episode(self, currEpisode, testID):
        ...
        # SAVING RESULTS
        ##############################################
        scenario_id = self.test_number

        np.save(f'result/routes/{scenario_id}_route.npy', route)
        np.save(f'result/graph/{scenario_id}_ground_truth.npy', self.env.ground_truth)

        # save self.node_coords
        np.save(f'result/graph/{scenario_id}_node_coords.npy', self.env.node_coords)

        # Save as JSON
        # Convert graph to a JSON-serializable dictionary
        graph_dict = self.env.prm.graph.to_dict()
        # Save the dictionary as a JSON file
        with open(f'result/graph/{scenario_id}_graph.json', 'w') as f:
