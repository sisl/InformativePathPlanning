function extract_path(ipp_problem::IPP, G::Vector{Any}, optimal_z_greedy, optimal_u_greedy, start::Int, goal::Int, all_pairs_shortest_paths::Graphs.FloydWarshallState{Float64, Int64}, dist::Matrix{Float64}, B::Int64)
    ...
    while (pos != goal) && (round(path_distance(ipp_problem, path) + path_distance(ipp_problem, shortest_path(all_pairs_shortest_paths, path[end], goal)), digits=3) <= B) # this can be <=B not whereas relaxed case needs <B
...
function run_AIPP_exact(ipp_problem::IPP, idx)
    tick()
    model = Model(
...
function run_DIPP_exact(ipp_problem::IPP, idx)
    tick()
    model = Model(
        optimizer_with_attributes(
            Pajarito.Optimizer,
...
function solve(ipp_problem::IPP, method::Exact)
    """ 
    ...
    if ipp_problem.objective == "A-IPP"
        path, objVal = run_AIPP_exact(ipp_problem, idx)
    ...
    elseif ipp_problem.objective == "D-IPP"
        ...
        error("Objective not recognized")
    end
end