...
function POMDPs.gen(bmdp::BeliefMDP, b, a, rng::AbstractRNG)
    s = RoverState(b.pos, zeros(bmdp.pomdp.query_size), b.cost_expended, b.drill_samples)
    if isterminal(bmdp.pomdp, s)
        bp = bmdp_handle_terminal(bmdp.pomdp, bmdp.updater, b, s, a, rng::AbstractRNG)::typeof(b)
        return (sp=bp, r=0.0)
    end
    _, o, r = @gen(:sp, :o, :r)(bmdp.pomdp, s, a, rng) # maybe this should have been generate_or?
